...
<div>
	...
	<form action="ChoisirCreneauServlet" method="post">
		<label for="magasin">Choisir un magasin :</label> <select
			name="magasin" id="magasin">
			<%
			...
			%>
		</select> <br> <label for="utiliserPoints">
			<%
			if (user.getPtFidelite() < 10) {
			%> Vous ne pouvez pas utiliser vos
			points de fidélité <%
			} else {
			%>Utiliser les points de fidélité :<%
			}
			%> (<span
			id="ptFidel"><%=user.getPtFidelite()%></span> pts)
		</label> <input type="checkbox" id="utiliserPoints" name="utiliserPoints"
			<%if (user.getPtFidelite() < 10) {%> disabled <%}%> value="true">
		<br> <input type="submit" value="Choisir le creneau">
	</form>
	<div>
		<p>
			Nombre de points utilisés : <span id="ptUsed">0</span>
		</p>
		<p>
